...
【生成するCore項目】
以下の項目を、指定された販売先市場・出力言語に合わせて、できるだけ具体的に記述してください。
...
- 入力言語に関係なく、生成結果本文・見出し・項目名・CTA・注意点は必ず指定された出力言語で書くこと
- 指定された販売先市場のECコピー、広告審査、消費者心理、価格・CTAの自然さに合わせること
- 直訳っぽい表現は避けること
- 商品の強みを活かした訴求にすること
"""
...
- 入力言語に関係なく、改善後のCore本文・見出し・項目名は指定された出力言語で書く
"""
...
class CoreEngine:
    ...
    def _format_core_options(self, options: Optional[dict] = None) -> str:
        ...
        target_market = str(options.get("target_market") or "japan").strip().lower()
        output_language = str(options.get("output_language") or "ja").strip().lower()
        market_note = str(options.get("market_note") or "").strip()
        ...
        }.get(target_market, "Japan / Japanese e-commerce market")
        lang_label = {
            "ja": "Japanese",
            "en": "English",
            "pt": "Portuguese (Brazil)",
        }.get(output_language, "Japanese")
        compliance = {
            "japan": "薬機法・景表法リスクを避け、日本の消費者に自然で丁寧な表現にする。",
            "us": "Write for US consumers. Avoid FTC/FDA-sensitive claims, unsupported guarantees, medical promises, fake scarcity, and overclaiming. Use benefit-led English, practical objections, concise CTAs, reviews/FAQ-style reassurance, and US ecommerce conventions.",
        ...
        }.get(target_market, "薬機法・景表法リスクを避け、日本の消費者に自然で丁寧な表現にする。")
        note_text = f"\n- 追加マーケット指示: {market_note}" if market_note else ""
        return (
            f"- 販売先市場: {market_label}\n"
            f"- 出力言語: {lang_label}\n"
            f"- 言語ルール: 入力言語に関係なく、本文・見出し・項目名・CTA・注意点は必ず出力言語で書く\n"
            f"- 市場・法規制・表現方針: {compliance}{note_text}\n"
            f"- 生成方針: {strategy}\n"
            f"- 安全度: {safety}\n"
            f"- 重視する要素: {focus_text or '商品ページ化しやすさ、差別化、悩み共感'}\n"
            f"- 文章トーン: {tone or '販売先市場に自然で信頼感のある表現'}"
        )
