...
def page_product_page():
    is_ja = st.session_state.get("lang", "ja") == "ja"
    st.markdown(
        '<div class="section-header">🛒 ' + ("Shopifyコード生成" if is_ja else "Gerar código Shopify") + '</div>',
        unsafe_allow_html=True,
    )
    ...
    with tab_liquid:
        ...
        st.markdown("#### " + ("1. 見た目を選ぶ" if is_ja else "1. Escolher aparência"))
        ...
        with rec_col2:
            if st.button("おすすめ設定を反映" if is_ja else "Aplicar recomendação", use_container_width=True, type="primary"):
        ...
        with st.container():
            ...
            with style_col1:
                mood = st.selectbox(
                    ...
                    format_func=lambda v: v if is_ja else MOOD_LABEL_PT.get(v, v),
                )
            with style_col2:
                palette = st.selectbox(
                    ...
                    format_func=lambda v: v if is_ja else PALETTE_LABEL_PT.get(v, v),
                )
            with style_col3:
                cta_strength = st.selectbox(
                    ...
                    format_func=lambda v: v if is_ja else CTA_LABEL_PT.get(v, v),
                )
            ...
            with st.expander("詳細設定（色・余白を細かく変える）" if is_ja else "Configurações avançadas (cores e espaçamento)", expanded=False):
                ...
                with layout_col1:
                    spacing = st.selectbox(
                        "余白" if is_ja else "Espaçamento",
                        ["コンパクト", "標準", "ゆったり"],
                        index=2,
                        key="liquid_design_spacing",
                        format_func=lambda v: v if is_ja else SPACING_LABEL_PT.get(v, v),
                    )
                with layout_col2:
            ...
            st.caption("この設定は次に生成するShopifyコードに反映されます。" if is_ja else "Estas configurações serão aplicadas ao próximo código Shopify gerado.")

        st.markdown("#### " + ("2. ページ構成を選ぶ" if is_ja else "2. Escolher estrutura da página"))
        ...
        with section_col1:
            section_preset = st.selectbox(
                ...
                format_func=lambda v: v if is_ja else SECTION_PRESET_LABEL_PT.get(v, v),
            )
            selected_section_keys = SECTION_PRESETS[section_preset]["keys"]
            st.caption(SECTION_PRESETS[section_preset]["description"] if is_ja else SECTION_PRESET_DESC_PT.get(section_preset, SECTION_PRESETS[section_preset]["description"]))
        with section_col2:
            section_labels = [
            ...
            st.markdown(
                '<div class="cs-info"><strong>' + ("この構成で使うセクション" if is_ja else "Seções usadas nesta estrutura") + '</strong><br>' +
                " / ".join(section_labels) +
                '</div>',
                unsafe_allow_html=True,
            )
        ...
        st.markdown("#### " + ("3. 生成する" if is_ja else "3. Gerar"))
        OUTPUT_LABEL_PT = {
        ...
        output_type = st.selectbox(
            "表示するコード" if is_ja else "Código exibido",
            output_options,
            key="liquid_output_type",
            format_func=lambda v: v if is_ja else OUTPUT_LABEL_PT.get(v, v),
        )
...
.ec-soon{font-size:0.68rem;color:#94a3b8;background:#151f2e;border:1px solid #334155;
  padding:2px 8px;border-radius:999px;margin-left:auto;flex-shrink:0;white-space:nowrap}
    ...
    if is_ja:
        ...
        export_summary_html = """
        <div class="td-action-grid">
          <div class="td-action-card"><strong>1. Conferir resultados</strong><span>Use as abas para alternar entre texto, Shopify, imagem, vídeo e anúncios.</span></div>
          <div class="td-action-card"><strong>2. Copiar ou baixar</strong><span>O que vai para Shopify deve sair pela aba Shopify.</span></div>
          <div class="td-action-card"><strong>3. Gerar o que falta</strong><span>Abas ainda vazias mostram um botão para ir à página de geração.</span></div>
        </div>
        """
    ...
    if is_ja:
        ...
        TAB_DEFS = [
            ("Texto",    "product_page",          "product_page"),
            ("Shopify",  "shopify_custom_liquid", "shopify_custom_liquid"),
            ("Imagem",   "image_prompt",          "image_prompt"),
            ("Vídeo",    "video_script",          "video_script"),
            ("Anúncios", "ads_sns",               "ads_sns"),
        ]
